...
// if targetValume has any value, it's 100
  ...
  // if it's already been played twice, then skip it entirely
  if(playcount[sig] && playcount[sig] >= 2){
      ...
      */    
      ...
      (function (thisAudibleDevice, thisPlayer) {
        ...
        thisPlayer.instance.on("end", function () {
          ...
          playcount[thisAudibleDevice.sig] = (playcount[thisAudibleDevice.sig] ? playcount[thisAudibleDevice.sig]++ : 0); // set it to 0 if it's not set, add one otherwise.
          if(player.numplays >= 2){ // if it's >= 2, remove it from AudibleDevices
