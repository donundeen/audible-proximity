...
const DEFAULT_MAX_CONCURRENT_PLAYERS = 5;
...
const PAUSE_BETWEEN_LOOPS = 15; // pause, in seconds, between loops of the same track
...
function handleRaddec(instance, raddec) {
 ...
 // console.log("sig " + sig +" : "+pre+" " + rssi+ " : " +kfrssi+": " + targetVolume +"\n");
    ...
    if(isAudible) {
      ...
      let pindex = AUDIO_FILE_LIST.players.indexOf(file);
  ...
  unplayedDevices.forEach((audibleDevice) => {
    for(let index = 0; index < availablePlayers.length; index++) {
      if(availablePlayers[index] === true) {
	      // this is where players are started and stopped, and the volume adjusted
        let player = instance.players[index];
	  ...
	  console.log("playing " + audibleDevice.file);
  ...
  players.forEach((player, index) => {
    if(player.file === audibleDevice.file) {
      ...
      if(player.instance.track === null) {
	      // PLAYERS ALSO PLAY HERE. HOW TO ADD A PAUSE AFTER A TRACK before playing it again?
        player.instance.play(audibleDevice.file);
      }
    }
  });
