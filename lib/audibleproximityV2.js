...
function showDebugScreen(audibleDevices){
  ...
  closestAudibleDevices.forEach(function (audibleDevice) {
    let player = audibleDevice.player;
    let report = audibleDevice.file.replace("data/audio/00000","") + " | " + (audibleDevice.isPlaying ? "P" : "") + (audibleDevice.isFading ? "F" : "") + " : " + Math.round(player.volume) + " : " + Math.round(audibleDevice.rssi) + " : " + (Date.now() - audibleDevice.lastRead);
    console.log(report);    
    content += report + "\n";
  });
