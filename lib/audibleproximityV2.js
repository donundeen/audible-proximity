...
const PAUSE_BETWEEN_LOOPS = 5000; // pause, in milliseconds, between loops of the same track
const DEFAULT_MAX_VOLUME_RSSI = -60;
const DEFAULT_MIN_VOLUME_RSSI = -80;
const BEACON_STALE_TIMEOUT_MS = 3000; // if beacon signal is older than this, it's "gone"
...
//      console.log(concurrencyCount + " out of " + DEFAULT_MAX_CONCURRENT_PLAYERS);
  ...
  */
  ...
  - if it's not playing, leave it
  */
      if(audibleDevice.isPlaying || audibleDevice.isFading){
        // do the fade.. Disconnect from AudibleDevice.targetVolume and jsut start fading
        player.instance.removeAllListeners("end"); // don't do the usualy end listener when we fade (ie, the wait then loop thing)
        audibleDevice.inWaitLoop = false;        
        audibleDevice.isPlaying = false;
        audibleDevice.isFading = true;
        // First loop: progressively reduce the volume of devices no longer detected
        player.volume  *= STALE_VOLUME_RATIO;
        if (player.volume  < 1) {
          player.volume  = 0;
        }
        console.log("fading "  + player.file + " : " +  player.volume + " : " +  audibleDevice.rssi);
        player.instance.volume(player.volume);
    //    console.log("INactive " + player.file + " : " + audibleDevice.kfrssi + " : " + player.volume);
        if ((audibleDevice.isPlaying == true || audibleDevice.isFading == true) && player.volume == 0) {
          console.log("stopping "  + player.file);
          player.instance.stop();
          audibleDevice.isFading = false;
          audibleDevice.isPlaying = false;
        }
      }
    }
...
function showDebugScreen(audibleDevices){
  ...
  closestAudibleDevices.forEach(function (audibleDevice) {
    ...
    let volume = Math.round(player.volume).toString().padStart(3,"0");
    let rssi = Math.round(audibleDevice.rssi).toString().padStart(4,"0");
    let report = audibleDevice.file.replace("data/audio/00000","") + " | " + state + " : " + volume + " : " + rssi + " : " + (Date.now() - audibleDevice.lastRead);
    console.log(report);    
    content += report + "\n";
  });
