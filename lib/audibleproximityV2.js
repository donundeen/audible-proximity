...
const DEFAULT_MAX_CONCURRENT_PLAYERS = 10;
...
*/
    ...
    */
    ...
    if((Date.now() - audibleDevice.lastRead) > BEACON_TIMEOUT_MS){
      targetVolume = 0; 
    }
  ...
  */
    ...
    //    console.log("INactive " + player.file + " : " + audibleDevice.kfrssi + " : " + player.volume);
        if ((audibleDevice.isPlaying == true || audibleDevice.isFading == true) && player.volume == 0) {
          // don't stop jsut because volume is 0
...
function showDebugScreen(audibleDevices){
  ...
  closestAudibleDevices.forEach(function (audibleDevice) {
    let player = audibleDevice.player;
    console.log(audibleDevice.file + " : " + (audibleDevice.isPlaying ? "P" : "") + (audibleDevice.isFading ? "F" : "") + " : " + player.volume + " : " + audibleDevice.rssi + " : " + (Date.now() - audibleDevice.lastRead));    
  });
}
