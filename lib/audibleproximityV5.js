...
class AudibleProximity {
  ...
  constructor(options) {
    ...
    waitForHeadphones(options, function () {
      ...
      self.audioFolderPath =
        options.audioFolderPath || DEFAULT_AUDIO_FOLDER_PATH;
      ...
      self.staleThresholdMilliseconds =
        options.staleThresholdMilliseconds ||
        DEFAULT_STALE_THRESHOLD_MILLISECONDS;
      self.packetProcessors =
        options.packetProcessors || DEFAULT_PACKET_PROCESSORS;
      self.packetInterpreters =
        options.packetInterpreters || DEFAULT_PACKET_INTERPRETERS;
      self.beaconStaleTimeoutMS =
        options.beaconStaleTimeoutMS || BEACON_STALE_TIMEOUT_MS;
      ...
      let pyoptions = { mode: 'text'}
      let pyshell = new PythonShell('/home/pi/audible-proximity/lib/getbeacondata.py', pyoptions);
...
//          console.log(data.toString());
 ...
 //           console.log(id + ":" + rssi + ":" + parseInt(rssi));
  ...
  //    pyshell.run('/home/pi/audible-proximity/lib/getbeacondata.py', pyoptions);
      /*
      let dummy = spawn("python", [
        "/home/pi/audible-proximity/lib/getbeacondata.py",
      ]);
...
*/
  ...
  if (process.env.HEADPHONES_ID) {
    ...
    exec(
      "bluetoothctl connect " + process.env.HEADPHONES_ID,
      (error, stdout, stderr) => {
        if (error) {
  ...
  exec("pacmd list-sinks", (error, stdout, stderr) => {
    ...
    if (stdout.includes("module-bluez5-device")) {
      ...
      //     console.log(`exec stdout ${stdout}`);
      setTimeout(function () {
        waitForHeadphones(options, callback);
      }, 1000);
    }
  });
}
...
function updateAudioPlayback(instance) {
  ...
  */
  let closestAudibleDevices = new Map(
    [...instance.audibleDevices.entries()].sort((a, b) => b[1].rssi - a[1].rssi)
  );
  ...
  closestAudibleDevices.forEach(function (audibleDevice) {
    ...
    */
    if (
      !audibleDevice.isPlaying &&
      !audibleDevice.inWaitLoop &&
      audibleDevice.targetVolume > 0
    ) {
...
function showDebugScreen(audibleDevices) {
  ...
  let closestAudibleDevices = new Map(
    [...audibleDevices.entries()].sort((a, b) => b[1].rssi - a[1].rssi)
  );
  ...
  closestAudibleDevices.forEach(function (audibleDevice) {
    ...
    let player = audibleDevice.player;
    let state = (
      (audibleDevice.isPlaying ? "P" : "") + (audibleDevice.isFading ? "F" : "")
    ).padStart(1, " ");
    let volume = Math.round(player.volume).toString().padStart(3, " ");
    let rssi = Math.round(audibleDevice.rssi).toString().padStart(4, " ");
    let duration = (Date.now() - audibleDevice.lastRead)
      .toString()
      .padEnd(5, " ");
    let report =
      audibleDevice.file.replace("data/audio/00000", "").replace(/\.mp3/i, "") +
      ":" +
      state +
      ":" +
      volume +
